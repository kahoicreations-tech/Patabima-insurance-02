...
class StaffUserProfile(Profile):
    ...
    # IRA (Insurance Regulatory Authority) Registration
    ira_number = models.CharField(
        max_length=50,
        unique=True,
        null=False,
        blank=False,
        help_text="Insurance Regulatory Authority registration number"
    )
    ira_verified = models.BooleanField(
        default=False,
        help_text="Whether the IRA number has been verified by admin"
    )
...
class CommercialTonnagePricing(BaseModel):
    ...
    tonnage_description = models.CharField(max_length=100)  # "Upto 3 Tons", "3.5 to 8 Tons", etc.
    base_premium = models.DecimalField(max_digits=12, decimal_places=2)
    
    # Fleet discount for multiple vehicles
    fleet_discount_percentage = models.DecimalField(max_digits=6, decimal_places=2, default=0)
...
class Campaign(BaseModel):
    ...
    message = models.TextField()
    image_url = models.URLField(max_length=500, blank=True)  # Increased to 500 for long S3 URLs
    # New: uploaded banner image (preferred over image_url when present)
    # Increase max_length to allow long original filenames without validation errors (default 100 was too short)
    banner_image = models.ImageField(upload_to='campaign_banners/', blank=True, null=True, max_length=255)
    call_to_action = models.CharField(max_length=100)
    action_url = models.URLField(max_length=500, blank=True)  # Increased to 500
...
class MotorPolicy(BaseModel):
    ...
    dmvic_certificate_preview_url = models.URLField(
        ...
        blank=True, 
        null=True,
        help_text="URL to DMVIC Certificate Preview PDF (from IntermediaryIntegration/PreviewTypeACertificate)"
    )
    ...
    # Manual Vehicle Valuation (Admin-driven)
    # Screens show a gating state "Pending Valuation" especially for Comprehensive.
    VALUATION_STATUS_CHOICES = [
        ('PENDING', 'Pending Valuation'),
        ('VALUED', 'Valuation Approved'),
        ('REJECTED', 'Valuation Rejected'),
    ]
    valuation_status = models.CharField(
        max_length=20,
        choices=VALUATION_STATUS_CHOICES,
        null=True,
        blank=True,
        default='PENDING',
        db_index=True,
        help_text="Manual valuation status set by admin for products requiring valuation",
    )
    valuation_marked_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When the admin marked the valuation status",
    )
    valuation_marked_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='motor_policies_valuations_marked',
        help_text="Admin user who marked valuation status",
    )
    valuation_notes = models.TextField(
        blank=True,
        default='',
        help_text="Admin notes about valuation (optional)",
    )
    ...
    def activate_policy(self, transaction_id, payment_date=None, payment_method='MPESA', run_post_tasks=True):
        """
        ...
        # Document generation is always attempted; notifications/commission can be skipped.
        try:
...
class Motor3Quotation(BaseModel):
    """Persisted Motor3 quotation (pre-policy) to support a complete quote lifecycle."""
    ...
    user = models.ForeignKey(User, on_delete=models.PROTECT, related_name='motor3_quotations')

    coverage_type = models.CharField(max_length=20, default='THIRD_PARTY', db_index=True)
    status = models.CharField(max_length=20, choices=QUOTATION_STATUS_CHOICES, default='DRAFT', db_index=True)

    registration_number = models.CharField(max_length=20, null=True, blank=True, db_index=True)
    proposed_cover_start_date = models.CharField(max_length=32, null=True, blank=True)

    transaction_id = models.CharField(max_length=128, null=True, blank=True, db_index=True)
    policy_number = models.CharField(max_length=50, null=True, blank=True, db_index=True)
...
class EmailVerificationCode(BaseModel):
    """
    Stores email verification codes sent to users via AWS SES.
    Codes expire after a configurable time period (default 15 minutes).
    """
    ...
    verified_at = models.DateTimeField(null=True, blank=True, help_text="When the code was successfully verified")
    ip_address = models.GenericIPAddressField(null=True, blank=True, help_text="IP address that requested verification")
