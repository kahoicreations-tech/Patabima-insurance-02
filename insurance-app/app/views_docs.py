...
def _boto_session():
    """Create a boto3 session, optionally pinned to AWS_PROFILE.

    This matches local/dev needs where the default shared-credentials profile
    might not have access to the uploads bucket.
    """
    profile = os.environ.get('AWS_PROFILE')
    region = _env('AWS_REGION') or os.environ.get('AWS_DEFAULT_REGION')
    if not profile:
        return boto3.Session(region_name=region)
    try:
        return boto3.Session(profile_name=profile, region_name=region)
    except Exception as e:
        logger.warning("[AWS] Failed to use AWS_PROFILE=%r: %s", profile, e)
        return boto3.Session(region_name=region)
...
def _normalize_textract_kv(resp: dict) -> dict:
    ...
    try:
        ...
        logger.debug("_normalize_textract_kv: Found %s blocks", len(blocks))
        ...
        logger.debug("_normalize_textract_kv: Found %s KEY blocks", len(keys))
        ...
        logger.debug("_normalize_textract_kv: Block types (sample): %s", block_types)
        ...
        logger.debug(
            "_normalize_textract_kv: Extracted %s fields (sample keys: %s)",
            len(fields),
            list(fields.keys())[:10],
        )
        return fields
    except Exception as e:
        logger.exception("_normalize_textract_kv exception: %s", e)
        return {}
...
def _canonicalize_fields(raw_fields: dict, all_lines: list[str], expected_doc_type: str | None = None) -> dict:
    ...
    # If we EXPECT a national ID (docType is known from upload), allow a conservative fallback
    # even when keywords are missing (e.g., cropped photos). Still avoid logbook-like docs.
    if (
        'id_number' not in canonical
        and (expected_doc_type or '').lower() in {'national_id', 'id', 'nationalid'}
        and not looks_logbook
    ):
        preferred_lines = [ln for ln in all_lines if re.search(r"\b(id|identity|identification)\b", ln, re.IGNORECASE)]
        scan_lines = preferred_lines + [ln for ln in all_lines if ln not in preferred_lines]
        for ln in scan_lines:
            for m in re_id_plain.finditer(ln):
                c = m.group(1)
                # Reject patterns like 12345678/90 or 12345678-xx
                if re.search(rf"{re.escape(c)}\s*[\-/]", ln):
                    continue
                canonical['id_number'] = c
                canonical.setdefault('id_type', 'national_id')
                break
            if 'id_number' in canonical:
                break
...
def _extract_fields(resp: dict, expected_doc_type: str | None = None) -> tuple[dict, dict]:
    """Return (raw_fields, canonical_fields) from Textract response."""
    logger.debug("_extract_fields: resp type=%s", type(resp))
    logger.debug(
        "_extract_fields: resp keys=%s",
        list(resp.keys()) if isinstance(resp, dict) else 'Not a dict',
    )
    
    raw_fields = _normalize_textract_kv(resp)
    logger.debug(
        "_extract_fields: _normalize_textract_kv extracted %s fields (keys sample=%s)",
        len(raw_fields),
        list(raw_fields.keys())[:25],
    )
    
    lines = _collect_text_lines(resp)
    logger.debug("_extract_fields: _collect_text_lines returned %s lines", len(lines))
    
    canonical = _canonicalize_fields({k.lower(): v for k, v in raw_fields.items()}, lines, expected_doc_type=expected_doc_type)
    logger.debug(
        "_extract_fields: _canonicalize_fields returned %s fields (keys=%s)",
        len(canonical),
        list(canonical.keys()),
    )
            ...
            for pattern in name_patterns:
                ...
                if match:
                    ...
                    if nm and len(nm) > 2:  # Must be at least 3 characters
                        ...
                        logger.debug("_extract_fields: extracted owner_name from lines=%r", nm)
                        break
...
def _maybe_complete_from_s3(doc: DocumentUpload):
    ...
    if not bucket:
        logger.warning("Docs pipeline: S3 bucket not set; cannot poll results")
        return False
    ...
    s3 = _boto_session().client('s3', region_name=_env('AWS_REGION'))
    raw = None
    found_key = None
    ...
    for key in candidate_keys:
        try:
            ...
            try:
                ...
                found_key = key
                logger.info("Docs pipeline: Found results at s3://%s/%s", bucket, key)
                break
            except Exception as read_err:
                logger.warning(
                    "Docs pipeline: get_object/read failed for s3://%s/%s: %s",
                    bucket,
                    key,
                    read_err,
                    exc_info=True,
    ...
    if raw is None:
        ...
        logger.info(
            "Docs pipeline: No results found for job %s in bucket %s. Tried %s locations.",
            doc.id,
            bucket,
            len(candidate_keys),
        )
    ...
    logger.debug("Docs pipeline: raw type=%s", type(raw))
    logger.debug(
        "Docs pipeline: raw keys=%s",
        list(raw.keys()) if isinstance(raw, dict) else 'Not a dict',
    )
    logger.debug(
        "Docs pipeline: raw has Blocks=%s",
        'Blocks' in raw if isinstance(raw, dict) else False,
    )

    # If Lambda wrote an error marker, mark FAILED and stop polling.
    if isinstance(raw, dict) and (raw.get('error') or raw.get('Error') or raw.get('errorMessage')):
        err = raw.get('error') or raw.get('Error') or {'message': raw.get('errorMessage')}
        with transaction.atomic():
            doc = DocumentUpload.objects.select_for_update().get(id=doc.id)
            doc.extracted_data = {'error': err, 'raw': raw}
            ...
            try:
                ...
                pass
        logger.warning(
            "Docs pipeline: Marked FAILED from error payload at s3://%s/%s",
            bucket,
            found_key,
        )
        return True
    
    raw_fields, canonical = _extract_fields(raw, expected_doc_type=doc.document_type)
    
    logger.debug("Docs pipeline: extracted raw_fields=%s", len(raw_fields))
    logger.debug("Docs pipeline: extracted canonical=%s", len(canonical))
    if canonical:
        logger.debug("Docs pipeline: canonical keys=%s", list(canonical.keys()))
...
class PresignUploadView(APIView):
    ...
    def post(self, request):
        try:
            ...
            region = _env('AWS_REGION') or os.environ.get('AWS_DEFAULT_REGION')
            s3 = _boto_session().client('s3', region_name=region, config=Config(signature_version='s3v4'))
            ...
            kms_key_id = _env('KMS_KEY_ID')
            sse_algo = 'aws:kms' if kms_key_id else 'AES256'
            ...
            presigned = s3.generate_presigned_url(
                ...
                Params={
                    ...
                    **({'SSEKMSKeyId': kms_key_id} if kms_key_id else {}),
                },
                ExpiresIn=int(_env('PRESIGN_EXPIRES_SEC', 600)),
            )

            # IMPORTANT: Any headers implied by signed Params MUST be sent by the client.
            # Otherwise S3 returns 403 AccessDenied / Signature mismatch.
            response_headers = {
                'Content-Type': mime_type,
                'x-amz-server-side-encryption': sse_algo,
            }
            if kms_key_id:
                response_headers['x-amz-server-side-encryption-aws-kms-key-id'] = kms_key_id
    ...
    def post(self, request):
        try:
            ...
            if queue_url and not _valid_queue(queue_url):
                ...
                logger.warning("Docs pipeline: SQS_QUEUE_URL appears invalid; skipping SQS enqueue")
                queue_url = None

            if queue_url:
                try:
                    sqs = _boto_session().client('sqs')
                    ...
                    # If base points to a local/emulator host, Lambda won't be able to call back.
                    # In that case, omit callbackUrl and rely on S3 results polling.
                    base_host = (base or '').lower()
                    use_callback = True
                    for bad in ('http://10.0.2.2', 'http://127.0.0.1', 'http://localhost', 'https://127.0.0.1', 'https://localhost'):
                        if base_host.startswith(bad):
                            use_callback = False
                            break
                    msg = {
                        'jobId': str(doc.id),
                    ...
                    if use_callback:
                        msg['callbackUrl'] = (base or '').rstrip('/') + '/api/v1/public_app/docs/callback'
                    ...
                    logger.warning("Docs pipeline: SQS send failed: %s", send_err, exc_info=True)
                    queue_url = None
            else:
            ...
            supports_extraction = dt in ('logbook', 'national_id', 'kra_pin')
...
class JobResultView(APIView):
    ...
    def get(self, request, job_id: str):
        ...
        if isinstance(data, dict) and 'canonicalFields' in data:
            canonical_out = data.get('canonicalFields') or {}
            raw_out = data.get('fields') or {}
            # If canonical exists but is empty, fall back to raw so callers don't get an empty result.
            fields_out = canonical_out or raw_out
            ...
            try:
                if isinstance(data, dict):
                    ...
                    canonical_only = _canonicalize_fields(lower, [], expected_doc_type=doc.document_type)
                    if canonical_only:
        ...
        try:
            with transaction.atomic():
                ...
                if provided_fields is not None or provided_canonical is not None:
                    ...
                    if 'Blocks' in result:
                        raw_fields, canonical = _extract_fields(result, expected_doc_type=doc.document_type)
...
class ApplyResultView(APIView):
    ...
    def post(self, request, job_id: str):
        ...
        if isinstance(data, dict) and 'canonicalFields' in data:
            ...
            try:
                if isinstance(data, dict):
                    canonical = _canonicalize_fields({str(k).lower(): v for k, v in data.items()}, [], expected_doc_type=doc.document_type)
            except Exception:
                pass
