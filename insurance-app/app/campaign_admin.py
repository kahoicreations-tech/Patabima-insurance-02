...
class CampaignSimpleForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # Require at least one: uploaded image or image_url
        image_url = cleaned.get('image_url')
        banner_image = cleaned.get('banner_image')
        if not banner_image and not image_url:
            self.add_error('banner_image', 'Provide a banner by uploading an image or entering an Image URL below')
        # Validate uploaded image dimensions/format/size for HomeScreen (but only if provided)
        if banner_image:
            try:
...
class CampaignAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Content', {
            ...
            'description': 'Upload a banner image (recommended) or provide an Image URL. Image URL is optional if you upload a banner. Recommended size ~16:9 (e.g. 1200x675). Use an online resizer/crop tool if needed.'
        }),
        ('Schedule', {
