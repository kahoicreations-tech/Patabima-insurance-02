...
# ============================================================================
# IRA NUMBER VERIFICATION EMAILS
# ============================================================================

def send_ira_verified_email(agent_email, agent_name, agent_code, ira_number):
    """
    Send email notification to agent when their IRA number is verified.
    
    Args:
        agent_email (str): Agent email address
        agent_name (str): Agent full name
        agent_code (str): Agent code
        ira_number (str): Verified IRA number
    ...
    try:
        ...
        logger.info(f"Sending IRA verification email to {agent_email}")
        
        # Prepare context for email template
        context = {
            'agent_name': agent_name or 'Valued Agent',
            'agent_code': agent_code,
            'ira_number': ira_number,
            'verification_date': datetime.now().strftime('%B %d, %Y'),
            'current_year': datetime.now().year
        }
        
        # Render HTML email
        html_content = render_to_string('emails/ira_verified.html', context)
        
        # Create email
        email = EmailMessage(
            subject=f'✓ IRA Number Verified - PataBima Agent {agent_code}',
            body=html_content,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[agent_email],
        )
        ...
        # Send email
        email.send()
        logger.info(f"IRA verification email sent successfully to {agent_email}")
        return True
        
    except Exception as e:
        logger.error(f"Error sending IRA verification email to {agent_email}: {e}")
        return False


def send_ira_rejected_email(agent_email, agent_name, agent_code, ira_number, reason=None):
    """
    Send email notification to agent when their IRA number requires verification.
    
    Args:
        agent_email (str): Agent email address
        agent_name (str): Agent full name
        agent_code (str): Agent code
        ira_number (str): IRA number that needs verification
        reason (str, optional): Reason for rejection/review
    ...
    try:
        ...
        logger.info(f"Sending IRA review notification email to {agent_email}")
        
        # Prepare context for email template
        context = {
            'agent_name': agent_name or 'Valued Agent',
            'agent_code': agent_code,
            'ira_number': ira_number or 'Not Provided',
            'review_date': datetime.now().strftime('%B %d, %Y'),
            'reason': reason or 'Please verify your IRA registration number is correct and active.',
            'current_year': datetime.now().year
        }
        
        # Render HTML email
        html_content = render_to_string('emails/ira_rejected.html', context)
        
        # Create email
        email = EmailMessage(
            subject=f'⚠ IRA Number - Action Required - PataBima Agent {agent_code}',
            body=html_content,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[agent_email],
        )
        ...
        # Send email
        email.send()
        logger.info(f"IRA review notification email sent successfully to {agent_email}")
        return True
        
    except Exception as e:
        logger.error(f"Error sending IRA review notification email to {agent_email}: {e}")
        return False