...
def create_commission_for_policy(policy: MotorPolicy) -> AgentCommission | None:
    """
    Create a single commission record for a policy.
    
    Args:
        policy: MotorPolicy instance
        
    Returns:
        AgentCommission instance if created, None if skipped
        
    Raises:
        ValueError: If policy is missing required data (agent, premium)
    """
    ...
    # Already has a commission
    if AgentCommission.objects.filter(policy=policy).exists():
        return AgentCommission.objects.filter(policy=policy).first()
    
    # Must have an agent
    if not policy.user:
        raise ValueError(f"Policy {policy.policy_number} has no agent (user)")
    
    # Extract premium
    premium_amount = _extract_premium(policy)
    if premium_amount <= 0:
        raise ValueError(f"Policy {policy.policy_number} has no valid premium amount")
