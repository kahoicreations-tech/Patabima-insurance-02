    ...
    def map_to_type_a_payload(policy_data: Dict) -> Dict[str, Any]:
        ...
        payload = {
            ...
            **({ 'IntermediaryIRANumber': getattr(settings, 'DMVIC_IRA_NUMBER', '') }
            ...
            # Also support common misspelling: chasisNumber (single 's')
            'Chassisnumber': DMVICFieldMapper.sanitize_alnum_upper(
                vehicle.get('chassis_number') or 
                vehicle.get('chassisNumber') or 
                vehicle.get('chasisNumber') or  # Common misspelling
                ''
            ),
            
            # RECOMMENDED OPTIONAL FIELDS (Better data quality)
            # Per Type A v1.8.1 request body table, use Vehiclemake/Vehiclemodel (lower-case 'm')
            ...
            # v1.8.1 notes Yearofregistration (Yes) and New Yearofmanufacture (No)
            # Provide both mapped from vehicle year to satisfy backends expecting either
            'Yearofregistration': int(vehicle.get('year', 0)) if vehicle.get('year') else 0,
