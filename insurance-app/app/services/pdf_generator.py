...
def generate_motor_policy_pdf(policy):
    ...
    try:
        ...
        # Upload to S3
        # Use a deterministic, semantically correct key name
        file_key = f"policies/{policy.policy_number}/{policy.policy_number}_policy.pdf"
        s3_url = upload_pdf_to_s3(pdf_content, policy.policy_number, file_key=file_key)
...
def upload_pdf_to_s3(pdf_content, policy_number, file_key=None):
    """
    ...
    try:
        ...
        file_key = file_key or f"policies/{policy_number}/{policy_number}_certificate.pdf"
...
def generate_motor_receipt_pdf(policy):
    """Generate a payment receipt PDF for a motor policy.

    Returns:
        str | None: S3 URL of uploaded receipt PDF.
    """
    try:
        from reportlab.lib.pagesizes import A4
        from reportlab.lib import colors
        ...
        doc = SimpleDocTemplate(
            buffer,
            pagesize=A4,
            title=f"Receipt {policy.policy_number}",
            author="PataBima",
            subject="Motor Insurance Receipt",
        )
        ...
        issue_dt = policy.approved_at or policy.submitted_at
        issue_date = issue_dt.strftime('%d %B %Y') if issue_dt else datetime.now().strftime('%d %B %Y')

        payment = policy.payment_details or {}
        premium = policy.premium_breakdown or {}
        client = policy.client_details or {}
        vehicle = policy.vehicle_details or {}

        receipt_data = [
            ['Receipt For Policy:', policy.policy_number],
            ['Issue Date:', issue_date],
            ['Payment Method:', payment.get('method') or payment.get('payment_method') or 'N/A'],
            ['Transaction ID:', payment.get('transaction_id') or payment.get('transactionId') or 'N/A'],
        ]

        receipt_table = Table(receipt_data, colWidths=[2.2*inch, 3.8*inch])
        ...
        client_name = client.get('fullName') or f"{client.get('firstName', '')} {client.get('lastName', '')}".strip() or 'N/A'
        client_data = [
            ['Name:', client_name],
            ['Phone:', client.get('phone') or client.get('phoneNumber') or 'N/A'],
            ['Email:', client.get('email') or 'N/A'],
            ['ID Number:', client.get('idNumber') or client.get('id_number') or 'N/A'],
        ]
        ...
        vehicle_data = [
            ['Registration:', vehicle.get('registration') or vehicle.get('registrationNumber') or vehicle.get('registration_number') or 'N/A'],
            ['Make:', vehicle.get('make') or 'N/A'],
            ['Model:', vehicle.get('model') or 'N/A'],
            ['Year:', str(vehicle.get('year') or vehicle.get('vehicle_year') or 'N/A')],
        ]
        vehicle_table = Table(vehicle_data, colWidths=[2.2*inch, 3.8*inch])
        vehicle_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (0, -1), colors.HexColor('#F0F0F0')),
            ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
            ('FONTSIZE', (0, 0), (-1, -1), 10),
        ...
        story.append(Paragraph("Amount", heading_style))
        total_amount = (
            premium.get('total_amount')
            or premium.get('totalAmount')
            or premium.get('total_premium')
            or premium.get('totalPremium')
            or payment.get('amount')
            or 0
        )
        base_premium = premium.get('base_premium') or premium.get('basePremium') or 0
        itl = premium.get('training_levy') or premium.get('trainingLevy') or premium.get('itl') or 0
        pcf = premium.get('pcf_levy') or premium.get('pcfLevy') or premium.get('pcf') or 0
        stamp = premium.get('stamp_duty') or premium.get('stampDuty') or 40

        amount_data = [
            ['Base Premium:', f"KSh {float(base_premium):,.2f}"],
            ['ITL (0.25%):', f"KSh {float(itl):,.2f}"],
            ['PCF (0.25%):', f"KSh {float(pcf):,.2f}"],
            ['Stamp Duty:', f"KSh {float(stamp):,.2f}"],
            ['', ''],
            ['TOTAL PAID:', f"KSh {float(total_amount):,.2f}"],
        ]
        amount_table = Table(amount_data, colWidths=[2.2*inch, 3.8*inch])
        ...
        file_key = f"policies/{policy.policy_number}/{policy.policy_number}_receipt.pdf"
        return upload_pdf_to_s3(pdf_content, policy.policy_number, file_key=file_key)

    except Exception as e:
        logger.error(f"Error generating receipt PDF for {getattr(policy, 'policy_number', 'UNKNOWN')}: {e}")
        return None
