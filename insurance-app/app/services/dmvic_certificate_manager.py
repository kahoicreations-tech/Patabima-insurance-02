...
class DMVICCertificateManager:
    ...
    @staticmethod
    def _is_non_retryable_dmvic_error(exc: Exception) -> bool:
        """Detect DMVIC errors that are unlikely to succeed on retry.

        In UAT/local we sometimes hit endpoint/version mismatches that consistently return:
        - ER001: "You tried invalid API"
        - HTTP 404: Resource not found

        Retrying these wastes ~50s and causes mobile request timeouts.
        """
        msg = (str(exc) or '').lower()
        if 'er001' in msg:
            return True
        if 'you tried invalid api' in msg or 'invalid api' in msg:
            return True
        if 'resource not found' in msg:
            return True
        # Some errors include: "DMVIC API error: 404 - ..."
        if 'dmvic api error' in msg and '404' in msg:
            return True
        return False
    ...
    def issue_certificate(
        ...
        for attempt in range(cls.MAX_RETRIES):
            try:
                ...
                dmvic_cert.dmvic_pdf_url = response.get('certificate_pdf_url') or response.get('pdf_url')
                ...
                policy.certificate_url = response.get('certificate_pdf_url') or response.get('pdf_url')
                ...
                # Fast-fail on non-retryable errors (endpoint/version mismatch, invalid API, etc.)
                if cls._is_non_retryable_dmvic_error(e):
                    logger.warning(
                        "⚠️ Detected non-retryable DMVIC error; skipping remaining retries to proceed with fallback"
                    )
        ...
        # All retries failed (or we exited early) - send admin alert and raise exception
