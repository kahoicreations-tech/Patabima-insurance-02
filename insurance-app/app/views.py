        ...
        with transaction.atomic():
            ...
            if role == 'CUSTOMER':
                ...
                models.StaffUserProfile.objects.create(
                    ...
                    full_names=full_names,
                    ira_number=f"PENDING-AGT{agent_code}",
                    ira_verified=False,
                )
...
class IntegrationsViewSet(BaseViewset):
    ...
    def vehicle_check(self, request):
        ...
        if dmvic_enabled:
            ...
            # DMVIC disabled - no simulation fallbacks.
            return Response(
                {
                    'success': False,
...
class PaymentsViewSet(BaseViewset):
    ...
    def initiate(self, request):
        ...
        # Optional references for downstream reconciliation
        ...
        if method != 'MPESA':
            ...
            return Response({'reference': reference, 'amount': amount_float, 'method': method, 'status': 'PENDING'}, status=status.HTTP_200_OK)
        ...
        try:
            from .services.mpesa_daraja import DarajaClient
    ...
    def status(self, request):
        ...
        try:
            ...
            # Daraja STK Query uses ResultCode '0' for success, other codes for pending/fail.
    ...
    def webhook(self, request):
        """Safaricom Daraja STK callback target (no auth)."""
        import logging

        logger = logging.getLogger(__name__)
        logger.info('M-PESA webhook received')
        logger.info('Webhook payload: %s', request.data)

        # Parse Daraja STK payload to our internal shape.
        try:
            from .services.mpesa_daraja import parse_stk_callback

            callback_info, metadata = parse_stk_callback(request.data if isinstance(request.data, dict) else {})
            result_code = str(callback_info.get('ResultCode') or '')
            ...
            unified = {
                ...
                'amount': None,
                'phone': None,
                'checkout_request_id': callback_info.get('CheckoutRequestID'),
                'merchant_request_id': callback_info.get('MerchantRequestID'),
                'raw': request.data,
            }

            if metadata:
                unified['amount'] = metadata.get('Amount')
                unified['transaction_id'] = metadata.get('MpesaReceiptNumber')
                unified['phone'] = metadata.get('PhoneNumber')

            # NOTE: STK push callback payload doesn't echo AccountReference.
            # For now we reconcile by checkout_request_id if needed.
            # Keep policy_reference unset unless client sent a custom mapping elsewhere.

            # Reuse the existing callback processing logic (includes DMVIC + activation).
            from .views.payment_gateway import process_payment_callback

            internal_request = request._request
            internal_request._body = None
            request._full_data = unified

            # Directly call the existing function-based view with the DRF request.
            return process_payment_callback(request)
        except Exception as e:
            logger.error('Failed to process M-PESA webhook: %s', e)
            # Return 200 to prevent Daraja retries from storming us.
            return Response({'ResultCode': 0, 'ResultDesc': 'received'}, status=status.HTTP_200_OK)
...
class NotificationsViewSet(BaseViewset):
    ...
    def fetch(self, request):
        ...
        Keep the payload intentionally small and UI-friendly; this endpoint is used
        by the mobile client to surface DMVIC certificate issuance updates.
        """
        try:
            from app.models import DMVICCertificate

            qs = (
                DMVICCertificate.objects
                .filter(motor_policy__user=request.user)
            ...
            for cert in qs[:50]:
                ...
                if status_code == 'ISSUED':
                    title = 'DMVIC Certificate Issued'
                    body = f"Certificate {cert_no or 'issued'} for policy {policy_number or ''}.".strip()
                elif status_code == 'FAILED':
                    title = 'DMVIC Certificate Failed'
                    body = f"Certificate issuance failed for policy {policy_number or ''}.".strip()
                elif status_code == 'CANCELLED':
                    title = 'DMVIC Certificate Cancelled'
                    body = f"Certificate was cancelled for policy {policy_number or ''}.".strip()
                else:
                    title = 'DMVIC Certificate Processing'
