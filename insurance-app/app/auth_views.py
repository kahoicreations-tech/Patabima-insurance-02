...
class LoginViewSet(BaseViewset):
    ...
    def signup(self, request):
        ...
        with transaction.atomic():
            ...
            ira_number = request.data.get('ira_number', None)
            ira_number = (str(ira_number).strip() if ira_number is not None else '')

            # IRA number is mandatory for AGENT accounts
            if role == 'AGENT':
                if not ira_number:
                    return Response(
                        {'ira_number': ['IRA number is required for agent accounts.']},
                        status=status.HTTP_400_BAD_REQUEST,
                    )

                # Basic uniqueness guard to avoid DB IntegrityError
                if models.StaffUserProfile.objects.filter(ira_number__iexact=ira_number).exists():
                    return Response(
                        {'ira_number': ['This IRA number is already registered.']},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            else:
                ira_number = None
            ...
            if role == 'CUSTOMER':
                ...
                models.StaffUserProfile.objects.create(
                    ...
                    full_names=full_names,
                    ira_number=ira_number,  # IRA number submitted during registration
                    ira_verified=False  # Admin must verify IRA number
                )
    ...
    def validate_phone(self, request):
        ...
        try:
            ...
            # Normalize Kenyan phone number to 10 digits with leading 0
            normalized_phone = phonenumber
            
            # Remove any non-digit characters
            cleaned = ''.join(filter(str.isdigit, phonenumber))
            
            # Handle different formats:
            # 254712345678 (12 digits) -> 0712345678 (10 digits)
            if cleaned.startswith('254') and len(cleaned) == 12:
                normalized_phone = '0' + cleaned[3:]
            # 712345678 (9 digits) -> 0712345678 (10 digits)
            elif len(cleaned) == 9:
                normalized_phone = '0' + cleaned
            # 0712345678 (10 digits) -> keep as is
            elif cleaned.startswith('0') and len(cleaned) == 10:
                normalized_phone = cleaned
            else:
                return Response({
                    'detail': 'Phone number must be Kenyan format: 0712345678 or 254712345678'
                }, status=status.HTTP_400_BAD_REQUEST)

            # Validate it's a proper Kenyan mobile number (starts with 07)
            if not normalized_phone.startswith('07'):
                return Response({
                    'detail': 'Phone number must be a Kenyan mobile number starting with 07'
                }, status=status.HTTP_400_BAD_REQUEST)

            # Check if phone number already exists (check both formats)
            if models.User.objects.filter(phonenumber=normalized_phone).exists() or \
               models.User.objects.filter(phonenumber=normalized_phone[1:]).exists():
                return Response({
