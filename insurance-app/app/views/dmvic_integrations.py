...
def verify_vehicle_with_dmvic(request):
    ...
    try:
        ...
        if cache_entry:
            is_valid = (timezone.now() - cache_entry.cached_at) <= timedelta(hours=24)
            if is_valid:
                cached_vehicle = (cache_entry.search_data or {}).get('vehicle')
                cached_existing_cover = (cache_entry.search_data or {}).get('existing_cover') or {
                ...
                return Response({
                    ...
                    'existing_cover': cached_existing_cover,
                    'cached': True,
    ...
    try:
        ...
        expiry = None
        policy = double_insurance.get('policy') if isinstance(double_insurance, dict) else None
        if isinstance(policy, dict):
            cover_end = policy.get('cover_end_date') or policy.get('cover_end')
            if isinstance(cover_end, str) and cover_end:
                try:
                    from datetime import datetime
                    expiry = datetime.strptime(cover_end.split('T')[0], '%Y-%m-%d').date()
                except Exception:
                    expiry = None
        DMVICVehicleSearch.objects.update_or_create(
            ...
            defaults={
                ...
                'has_existing_cover': bool(double_insurance.get('exists', False)),
                'existing_cover_expiry': expiry or None,
