...
def _compute_premium_from_db(underwriter_code: str, category_code: str, product_code: str, payload: dict):
    ...
    # PASSENGER-BASED: PSV variants - Use PSVPLLPricing table
    if (sub.pricing_model or '').upper() == 'PASSENGER':
        try:
            passenger_capacity = int(payload.get('passenger_capacity') or payload.get('capacity') or 0)
            pll_amount = Decimal(str(payload.get('pll_amount') or '500'))  # Default to 500
        except Exception:
            # If passenger data missing, try base MotorPricing as fallback
        ...
        # Look up PSV PLL pricing
        pll_pricing = PSVPLLPricing.objects.filter(
            subcategory=sub,
            underwriter=uw,
        ...
        if pll_pricing and passenger_capacity > 0:
            # Calculate: rate_per_person * passenger_capacity
            base_premium = (Decimal(str(pll_pricing.rate_per_person)) * Decimal(passenger_capacity)).quantize(Decimal('1.00'))
            return base_premium
        
        # Fallback to MotorPricing if PSVPLLPricing not found
        mp = MotorPricing.objects.filter(
            subcategory=sub,
            underwriter=uw,
            is_active=True,
...
def compare_pricing(request):
    ...
    for uwd in all_underwriters:
        ...
        if sub:
            has_db = (
                ...
                CommercialTonnagePricing.objects.filter(subcategory=sub, underwriter=uo, is_active=True).exists() or
                PSVPLLPricing.objects.filter(subcategory=sub, underwriter=uo, is_active=True).exists()
            )
    ...
    for comp in comparisons:
        ...
        if result.get('_exclude_from_ext_product'):
            logger.info(
                "Excluding %s - no extendible_config for %s",
                result.get('underwriter_name'),
                subcategory_code,
            )
            continue
    ...
    logger.info(
        "Comparison results: %s out of %s underwriters shown for %s",
        len(filtered_comparisons),
        len(comparisons),
        subcategory_code,
    )
