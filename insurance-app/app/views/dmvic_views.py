...
def search_vehicle(request):
    ...
    try:
        ...
        # Authentication/config/certificate issues should not be treated as an internal server error.
        logger.error(f"DMVIC authentication error during vehicle search for {registration_number}: {str(e)}", exc_info=True)
        return Response(
            {"success": False, "error": "DMVIC authentication failed", "detail": str(e)},
            status=status.HTTP_503_SERVICE_UNAVAILABLE,
        )
    except DMVICAPIError as e:
        # DMVIC not-found is an expected business outcome; return 404 (not 500)
        msg = str(e)
        logger.warning(f"DMVIC vehicle search failed for {registration_number}: {msg}")

        if 'not found in DMVIC database' in msg:
            return Response(
                {"success": False, "error": msg, "code": "DMVIC_VEHICLE_NOT_FOUND"},
                status=status.HTTP_404_NOT_FOUND,
            )

        # Network/upstream failures should be surfaced as bad gateway
        lowered = msg.lower()
        if 'network error' in lowered or 'timed out' in lowered or 'timeout' in lowered:
            return Response(
                {"success": False, "error": "DMVIC service unreachable", "detail": msg},
...
def preview_certificate(request):
    ...
    try:
        ...
        # Generate preview using Intermediary Integration
        dmvic_service = DMVICService()
        
        # Build policy_data dictionary for Intermediary API
        policy_data = {
        ...
        # Use Intermediary Integration endpoint (works for all certificate types)
        logger.info(f"Using Intermediary Integration for preview: {policy.policy_number}")
        ...
        if result['success']:
            return Response({
                ...
                "message": "Preview generated using Intermediary Integration"
            }, status=status.HTTP_200_OK)
        else:
            return Response({
                "success": False,
...
def issue_certificate(request):
    ...
    try:
        ...
        # Issue certificate using Intermediary Integration
        dmvic_service = DMVICService()
        ...
        # Use Intermediary Integration endpoint (works for all certificate types)
        logger.info(f"Using Intermediary Integration for issuance: {policy.policy_number}")
        result = dmvic_service.issue_type_a_certificate_intermediary(policy_data)
        ...
        if result['success']:
            ...
            policy.dmvic_ref_no = result['ref_no']
            policy.dmvic_certificate_pdf_url = result.get('certificate_pdf_url')
            policy.dmvic_certificate_type = certificate_type
            policy.dmvic_issued_at = timezone.now()
            policy.status = 'ACTIVE'
            policy.save()
            
            logger.info(f"Certificate {result['certificate_number']} issued successfully via Intermediary")
            ...
            return Response({
                ...
                "certificate_pdf_url": result.get('certificate_pdf_url'),
                "message": f"Type {certificate_type} certificate issued successfully (Intermediary Integration)"
            }, status=status.HTTP_200_OK)
        else:
            # Check for ER006 (no inventory)
            has_er006 = any(err.get('ErrorCode') == 'ER006' for err in result['errors'])
