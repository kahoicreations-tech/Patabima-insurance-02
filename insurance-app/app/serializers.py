...
class UserSerializer(serializers.ModelSerializer):
    ...
    ira_number = serializers.SerializerMethodField()
    ira_verified = serializers.SerializerMethodField()
    ...
    def get_ira_number(self, obj):
        """Get IRA number for agents"""
        if obj.role == 'CUSTOMER':
            return None
        if hasattr(obj, 'staff_user_profile') and obj.staff_user_profile:
            return obj.staff_user_profile.ira_number
        return None

    def get_ira_verified(self, obj):
        """Get IRA verification status for agents"""
        if obj.role == 'CUSTOMER':
            return None
        if hasattr(obj, 'staff_user_profile') and obj.staff_user_profile:
            return obj.staff_user_profile.ira_verified
        return False
    ...
    def get_is_email_verified(self, obj):
        ...
        if obj.role == 'CUSTOMER':
            if hasattr(obj, 'public_user_profile') and obj.public_user_profile:
                return obj.public_user_profile.is_email_verified
        else:
            if hasattr(obj, 'staff_user_profile') and obj.staff_user_profile:
                return obj.staff_user_profile.is_email_verified
        return False
