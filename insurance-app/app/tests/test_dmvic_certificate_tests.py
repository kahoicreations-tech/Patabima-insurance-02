...
class DMVICVehicleSearchTests(TestCase):
    ...
    def test_vehicle_search_response_kda234h(self, mock_request):
        ...
        # Mock DMVIC API response in the common callbackObj shape
        mock_request.return_value = {
            'callbackObj': {
                'Vehicle': [
                    {
                        'VehicleRegistrationNumber': 'KDA234H',
        ...
        self.assertEqual(result['vehicle_type'], 'SD')
        ...
        self.assertEqual(payload['VehicleRegistrationNumber'], 'KDA234H')
    
    @patch('app.services.dmvic_service.DMVICService._make_authenticated_request')
    def test_vehicle_search_commercial_vehicle(self, mock_request):
        """Test vehicle search for commercial vehicle with tonnage"""
        
        # Mock commercial vehicle response in callbackObj shape
        mock_request.return_value = {
            'callbackObj': {
                'Vehicle': [
                    {
                        'VehicleRegistrationNumber': 'KBZ789T',
        ...
        self.assertEqual(result['vehicle_type'], 'TR')
...
class DMVICCertificateIssuanceTests(TestCase):
    ...
    def test_issue_type_a_certificate_third_party(self, mock_request):
        ...
        self.assertEqual(result['transaction_no'], 'Q-AA0108')
        self.assertEqual(result['api_request_number'], 'O-AA0024')
        self.assertEqual(result['email'], 'client@example.com')
    ...
    def test_issue_type_b_certificate_comprehensive(self, mock_request):
        ...
        mock_request.return_value = {
            'success': True,
            'APIRequestNumber': 'O-BB0024',
            'callbackObj': {
                'issueCertificate': {
                    'TransactionNo': 'Q-BB0108',
                    'actualCNo': 'CHB987654',
                    'Email': 'client@example.com',
                }
            }
        ...
        self.assertEqual(result['transaction_no'], 'Q-BB0108')
        self.assertEqual(result['api_request_number'], 'O-BB0024')
        self.assertEqual(result['status'], 'ACTIVE')
