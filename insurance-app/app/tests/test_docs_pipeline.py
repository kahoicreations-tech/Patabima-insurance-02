...
class DocsPipelineTests(TestCase):
    def setUp(self):
        ...
        # Use explicit assignment (not setdefault) so local dev env vars
        # don't leak into tests and trigger real S3 polling.
        os.environ['S3_BUCKET'] = 'test-bucket'
        os.environ['PRESIGN_EXPIRES_SEC'] = '600'
        os.environ['MAX_UPLOAD_MB'] = '15'
        # Ensure callback auth is disabled for this test.
        os.environ['CALLBACK_SECRET'] = ''
        os.environ['DOCS_HMAC_SECRET'] = ''

        self.user_model = get_user_model()
        self.user = self.user_model.objects.create_user(
            phonenumber='712345678', password='pass1234', email='t@example.com'
        )
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)

    @mock.patch('app.views_docs._boto_session')
    def test_end_to_end_without_aws(self, mock_boto_session):
        # Mock S3 client used by views_docs via boto3.Session().client(...)
        s3_instance = mock.Mock()
        s3_instance.generate_presigned_url.return_value = 'https://s3.fake/presigned'
        # Ensure status polling does not find any results in S3
        s3_instance.head_object.side_effect = Exception('NotFound')
        session_instance = mock.Mock()
        session_instance.client.return_value = s3_instance
