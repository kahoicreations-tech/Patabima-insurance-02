...
class IRAVerificationFilter(admin.SimpleListFilter):
    """Filter agents by IRA verification status."""
    title = 'IRA Verification Status'
    parameter_name = 'ira_verified'

    def lookups(self, request, model_admin):
        return (
            ('verified', '✓ Verified'),
            ('pending', '⚠ Pending Verification'),
            ('no_ira', 'No IRA Number'),
        )
    ...
    def queryset(self, request, queryset):
        ...
        if val == 'verified':
            return queryset.filter(
                staff_user_profile__ira_number__isnull=False,
                staff_user_profile__ira_verified=True
            )
        if val == 'pending':
            return queryset.filter(
                staff_user_profile__ira_number__isnull=False,
                staff_user_profile__ira_verified=False
            )
        if val == 'no_ira':
            return queryset.filter(
                staff_user_profile__ira_number__isnull=True
            ) | queryset.filter(
                staff_user_profile__ira_number=''
            )
        return queryset
...
class MotorPolicyAdmin(admin.ModelAdmin):
    ...
    actions = [
        'mark_valuation_valued',
        'mark_valuation_pending',
        'generate_commissions_for_policies',
        'activate_policies',
        'export_policies',
    ]
    ...
    fieldsets = (
        ...
        ('Valuation (Manual)', {
            'fields': ('valuation_status', 'valuation_marked_at', 'valuation_marked_by', 'valuation_notes'),
            'description': 'Manual valuation gating (admin marks whether valuation is approved).',
            'classes': ('collapse',),
        }),
        ('Client Details', {
    ...
    # ============================================================================
    # Admin Actions - Manual Valuation
    # ============================================================================

    @admin.action(description='Mark valuation as APPROVED (VALUED)')
    def mark_valuation_valued(self, request, queryset):
        now = timezone.now()
        updated = queryset.update(
            valuation_status='VALUED',
            valuation_marked_at=now,
            valuation_marked_by=request.user,
        )
        self.message_user(request, f"Marked {updated} policy(ies) as VALUED.", level=messages.SUCCESS)

    @admin.action(description='Mark valuation as PENDING')
    def mark_valuation_pending(self, request, queryset):
        now = timezone.now()
        updated = queryset.update(
        ...
        self.message_user(request, f"Marked {updated} policy(ies) as PENDING.", level=messages.SUCCESS)
...
class EnhancedUserAdmin(admin.ModelAdmin):
    ...
    def ira_status_display(self, obj):
        """Display IRA number and verification status."""
        if hasattr(obj, 'staff_user_profile') and obj.staff_user_profile:
            profile = obj.staff_user_profile
            ira = profile.ira_number or 'Not Provided'
            
            if not profile.ira_number:
                return format_html(
                    '<span style="color: #9CA3AF; font-size: 11px;">●</span> <span style="color: #6B7280;">No IRA</span>'
                )
            elif profile.ira_verified:
                return format_html(
        ...
        return '-'
    ira_status_display.short_description = 'IRA Number'
        ...
        for user in queryset:
            if not hasattr(user, 'staff_user_profile') or not user.staff_user_profile:
                ...
                StaffUserProfile.objects.create(
                    ...
                    ira_number=f"PENDING-AGT{str(next_code).zfill(4)}",
                    ira_verified=False,
                )
                user.is_staff = True
                user.role = 'AGENT'
    ...
    def verify_ira_numbers(self, request, queryset):
        """Verify IRA numbers for selected agents and send confirmation emails."""
        from .services.notifications import send_ira_verified_email
        
        verified = 0
        emailed = 0
        
        for user in queryset:
            # Only process agents with staff profiles
            if hasattr(user, 'staff_user_profile') and user.staff_user_profile:
                profile = user.staff_user_profile
                
                # Skip if no IRA number provided
                if not profile.ira_number:
                    continue
                
                # Set as verified
                profile.ira_verified = True
                profile.save(update_fields=['ira_verified'])
                verified += 1
                
                # Send verification email if email exists
                if user.email:
                    agent_code = f"{profile.agent_prefix}{profile.agent_code}"
                    success = send_ira_verified_email(
                        agent_email=user.email,
                        agent_name=profile.full_names,
                        agent_code=agent_code,
                        ira_number=profile.ira_number
                    )
                    if success:
                        emailed += 1
        ...
        if verified > 0:
            self.message_user(
                request,
                f"✓ Verified IRA numbers for {verified} agent(s). {emailed} confirmation email(s) sent.",
                level=messages.SUCCESS
            )
        else:
            self.message_user(
                request,
                "No agents with IRA numbers found in selection.",
                level=messages.WARNING
            )
    
    verify_ira_numbers.short_description = "✓ Verify IRA numbers for selected agents"
    
    def reject_ira_numbers(self, request, queryset):
        """Reject/unverify IRA numbers for selected agents and send notification emails."""
        from .services.notifications import send_ira_rejected_email
        ...
        for user in queryset:
            ...
            if hasattr(user, 'staff_user_profile') and user.staff_user_profile:
                ...
                if user.email:
                    agent_code = f"{profile.agent_prefix}{profile.agent_code}"
                    success = send_ira_rejected_email(
                        agent_email=user.email,
                        agent_name=profile.full_names,
                        agent_code=agent_code,
                        ira_number=profile.ira_number,
                        reason="Please verify your IRA registration number and ensure it is active and up-to-date."
                    )
                    if success:
                        emailed += 1
        ...
        if rejected > 0:
            self.message_user(
                request,
                f"⚠ Unverified IRA numbers for {rejected} agent(s). {emailed} notification email(s) sent.",
                level=messages.WARNING
            )
        else:
            self.message_user(
    ...
    reject_ira_numbers.short_description = "⚠ Reject/Unverify IRA numbers for selected agents"
