  ...
  const submitPolicy = async () => {
    try {
      ...
      try {
        ...
        // Handle DMVIC Certificate Issuance Failure (400 status)
        if (apiError.status === 400 || apiError.statusCode === 400) {
          const errorData = apiError.payload || apiError.response?.data || {};
          
          // Check if it's a DMVIC certificate failure
          if (errorData.errorCode === 'DMVIC_CERTIFICATE_FAILED') {
            console.error('[PolicySubmission] ❌ DMVIC CERTIFICATE FAILED:', errorData);
            
            // Clear guard to allow retry
            await AsyncStorage.removeItem('policy_submission_guard');
            
            // Show error with DMVIC's actual error message
            Alert.alert(
              '❌ DMVIC Certificate Issuance Failed',
              `${errorData.message || errorData.dmvicError || 'Unable to issue certificate from DMVIC'}\n\n` +
              `Policy Number: ${errorData.policyNumber || 'N/A'} (Draft - Not Activated)\n\n` +
              `${errorData.support ? `Contact Support:\n${errorData.support.phone}\n${errorData.support.email}\n${errorData.support.hours}` : 'Please contact support.'}`,
              [
                {
                  text: 'Contact Support',
                  onPress: () => {
                    Alert.alert(
                      'Support Contact',
                      errorData.support ? 
                        `Phone: ${errorData.support.phone}\nEmail: ${errorData.support.email}\nHours: ${errorData.support.hours}` :
                ...
                },
                {
                  ...
                  onPress: () => {
                    ...
                    if (onSubmissionError) {
                      onSubmissionError(new Error(errorData.message || errorData.dmvicError || 'DMVIC certificate issuance failed'));
                    } else {
                      navigation.goBack();
                    }
                  }
      ...
      const result = {
        ...
        warning: response.warning || null,  // Pass backend warning to success screen
        testModeWarning: response.testModeWarning || null  // Pass test mode warning
      };
