...
/**
 * Helper: Format human-friendly label from subcategory data
 */
const getDisplayLabel = (item) => {
  // Priority 1: Use public_label if available
  if (item.public_label) {
    return item.public_label;
  }
  
  // Priority 2: Use name if it's not same as code
  if (item.name && item.name !== item.subcategory_code) {
    return item.name;
  }
  
  // Priority 3: Prettify subcategory_code
  ...
  return code
    ...
    .map((word) => {
      ...
      if (['TP', 'TOR', 'COMP', 'PSV', 'PM', 'EXT', 'KG'].includes(upperWord)) {
        return upperWord;
      }
