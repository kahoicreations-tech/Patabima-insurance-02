        ...
        return {
            ...
            'breakdown': {
                ...
                'totals': {
                    ...
                    'gross_premium': float(gross_premium),
                    'mandatory_levies': float(itl_levy + pcf_levy + stamp_duty + service_fee),
        ...
        categories_data = [
            {
                ...
                'subcategories': [
                    ...
                    {
                        'subcategory_code': 'motor_private_time_on_risk',
                        'subcategory_name': 'Motor Private Time on Risk cover',
                        'product_type': 'time_on_risk',
                        'additional_fields': [],
                        'pricing_requirements': {},
                        'fixed_premium': 650
                    },
                    {
                        'subcategory_code': 'motor_commercial_time_on_risk',
                        'subcategory_name': 'Motor Commercial Time on Risk cover',
                        'product_type': 'time_on_risk',
                        'additional_fields': [],
                        'pricing_requirements': {},
                        'fixed_premium': 850
            ...
            },
            {
                ...
                'subcategories': [
                    ...
                    {
                        'subcategory_code': 'commercial_tpo_tonnage_scale',
                        'subcategory_name': 'Motor Commercial TPO Tonnage Scale',
                        'product_type': 'third_party',
                        'additional_fields': ['tonnage', 'is_fleet'],
                        'pricing_requirements': {
                            'tonnage': {'required': True, 'type': 'number', 'min': 0, 'max': 50},
                            'is_fleet': {'required': False, 'type': 'boolean'}
                        }
                    },
                    {
                        'subcategory_code': 'commercial_tpo_tonnage_scale_fleet',
                        'subcategory_name': 'Motor Commercial TPO Tonnage Scale FLEET',
                        'product_type': 'third_party',
                        'additional_fields': ['tonnage'],
                        'pricing_requirements': {
                            'tonnage': {'required': True, 'type': 'number', 'min': 0, 'max': 50}
                    ...
                    {
                        'subcategory_code': 'prime_mover',
                        'subcategory_name': 'Prime Mover',
                        'product_type': 'third_party',
                        'additional_fields': [],
                        'pricing_requirements': {},
                        'fixed_premium': 10000
            ...
            },
            {
                ...
                'subcategories': [
                    ...
                    {
                        'subcategory_code': 'motor_psv_chauffeurdriven_tor',
                        'subcategory_name': 'Motor PSV chauffeurdriven T.O.R',
                        'product_type': 'tor',
                        'additional_fields': ['passenger_capacity'],
                        'pricing_requirements': {
                            'passenger_capacity': {'required': True, 'type': 'number', 'min': 1, 'max': 50}
                        ...
                        'base_premium': 1000,
                        'pll_rate_per_passenger': 100
                    },
                    {
                        'subcategory_code': 'motorcycle_psv_tor',
                        'subcategory_name': 'Motorcycle PSV T.O.R',
                        'product_type': 'tor',
                        'additional_fields': [],
                        'pricing_requirements': {},
                        'fixed_premium': 500
                    ...
                    {
                        'subcategory_code': 'pll_500_all_psv',
                        'subcategory_name': 'PLL @ 500 Per person for All PSVs',
                        'product_type': 'pll',
                        'additional_fields': ['passenger_capacity'],
                        'pricing_requirements': {
                            'passenger_capacity': {'required': True, 'type': 'number', 'min': 1, 'max': 100}
                        },
                        'rate_per_person': 500
                    }
                ]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Seed Commercial Tonnage pricing
        self._seed_commercial_tonnage_pricing()

        # Seed Comprehensive pricing (bracket-based)
        self._seed_comprehensive_pricing()

        # Seed PSV PLL pricing
        self._seed_psv_pll_pricing()
        ...
        for pricing in tor_pricing_data:
            ...
            MotorPricing.objects.get_or_create(
                ...
                defaults={
                    ...
                    'insurance_training_levy_rate': Decimal('0.0025'),  # 0.25%
                    'pcf_levy_rate': Decimal('0.0025'),  # 0.25%
                    'stamp_duty': Decimal('40'),
                    'effective_from': '2024-01-01'
                }
            )

    def _seed_commercial_tonnage_pricing(self):
        """Seed commercial tonnage scale pricing"""
        jubilee = Underwriters.objects.get(underwriter_code='jubilee')

        # Get commercial tonnage subcategories
        commercial_tonnage_subcategories = [
            'commercial_tpo_tonnage_scale',
            'commercial_tpo_tonnage_scale_fleet'
        ]

        # Commercial tonnage brackets (from the image)
        tonnage_brackets = [
            {'min': 0, 'max': 3, 'description': 'Upto 3 Tons', 'premium': 4500},
            {'min': 3.5, 'max': 8, 'description': '3.5 to 8 Tons', 'premium': 5500},
            {'min': 8.5, 'max': 12, 'description': '8.5 to 12 Tons', 'premium': 6500},
            {'min': 12.5, 'max': 15, 'description': '12.5 to 15 Tons', 'premium': 7500},
            {'min': 15.5, 'max': 20, 'description': '15.5 to 20 Tons', 'premium': 10000},
            {'min': 20.1, 'max': 100, 'description': 'Over 20 Tons', 'premium': 15000},
        ]

        for sub_code in commercial_tonnage_subcategories:
            try:
                subcategory = MotorSubcategories.objects.get(subcategory_code=sub_code)
                is_fleet = 'fleet' in sub_code.lower()

                for bracket in tonnage_brackets:
                    CommercialTonnagePricing.objects.get_or_create(
                        underwriter=jubilee,
                        subcategory=subcategory,
                        tonnage_min=Decimal(str(bracket['min'])),
                        tonnage_max=Decimal(str(bracket['max'])),
                        defaults={
                            'tonnage_description': bracket['description'],
                            'base_premium': Decimal(str(bracket['premium'])),
                            'fleet_discount_percentage': Decimal('5') if is_fleet else Decimal('0'),
                            'is_fleet_pricing': is_fleet,
                            'effective_from': '2024-01-01'
                        }
                    )

                # Add Prime Mover pricing
                CommercialTonnagePricing.objects.get_or_create(
                    underwriter=jubilee,
                    subcategory=subcategory,
                    tonnage_min=Decimal('0'),
                    tonnage_max=Decimal('999'),
                    defaults={
                        'tonnage_description': 'Prime mover',
            ...
            except MotorSubcategories.DoesNotExist:
                print(f"Subcategory {sub_code} not found, skipping...")
                continue

    def _seed_psv_pll_pricing(self):
        """Seed PSV PLL (Personal Liability Limit) pricing"""
        jubilee = Underwriters.objects.get(underwriter_code='jubilee')

        # PLL pricing options
        pll_options = [
            {'vehicle_type': 'psv_general', 'rate_per_person': 500, 'description': 'PLL @ 500 Per person for All PSVs'},
            {'vehicle_type': 'commercial_institutional', 'rate_per_person': 250, 'description': 'PLL @ 250 Per person for Motor commercial Institutional'},
        ]

        for pll in pll_options:
            PSVPLLPricing.objects.get_or_create(
                underwriter=jubilee,
                vehicle_type=pll['vehicle_type'],
                defaults={
                    'pll_rate_per_person': Decimal(str(pll['rate_per_person'])),
