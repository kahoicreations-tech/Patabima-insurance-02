...
- **Database Engine**: PostgreSQL (psycopg 3.x driver)
- **Database Name**: `patabima_insurance`
- **Host**: `127.0.0.1` (localhost for development)
- **Django Project**: `insurance` (not `insurance-app`)
- **Settings Module**: `insurance.settings`
- **Migrations App**: `app`

### Database Tables (Key Motor 2 Tables)

- `app_motorcategory` - Motor insurance categories (PRIVATE, COMMERCIAL, PSV, MOTORCYCLE, TUKTUK, SPECIAL)
- `app_motorsubcategory` - Subcategories with pricing models (FIXED, BRACKET, TONNAGE, PASSENGER)
- `app_insuranceprovider` - Underwriters/insurance providers
- `app_motorpricing` - Base pricing for FIXED and BRACKET models
- `app_commercialtonnagepricing` - Tonnage-based pricing for commercial vehicles
  - **Required Fields**: `fleet_discount_percentage` (Decimal, NOT NULL)
  - Structure: `tonnage_from`, `tonnage_to`, `tonnage_description`, `base_premium`, `fleet_discount_percentage`, `is_prime_mover`, `is_over_limit`
- `app_psvpllpricing` - PSV Passenger Legal Liability pricing
  - Structure: `pll_amount`, `rate_per_person`, `is_commercial_institutional`

### Motor 2 Pricing Status (Last Updated: November 27, 2025)

**Status**: ✅ **FULLY POPULATED**

The database contains comprehensive pricing for all Motor 2 subcategories:

- **7 Underwriters**: Madison, PATABIMA, Jubilee, UAP, APA, Britam, CIC
- **68 Active Subcategories** across 6 categories
- **748 Total Pricing Records**:
  - Fixed/Bracket Pricing: 176 records
  - Tonnage Pricing: 404 records (Commercial vehicles)
  - Passenger Pricing: 168 records (PSV/TukTuk)

**Price Tiers**:

- LOW (Madison, PATABIMA, Jubilee): Base price (1.0x multiplier)
- MEDIUM (UAP, APA): +18% (1.18x multiplier)
- HIGH (Britam, CIC): +32% (1.32x multiplier)

**To Repopulate Pricing**:

```powershell
cd insurance-app
python -c "import os, sys, django; os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'insurance.settings'); django.setup(); exec(open('../scripts/populate_motor2_pricing.py').read())"
```

### Migration Best Practices

1. **Always check existing database schema** before creating migrations:
...
2. **Django Settings Module**: Use `insurance.settings` not `insurance-app.settings`

3. **Required Fields Check**: Always verify required fields in database before populating:

   - `app_commercialtonnagepricing` requires `fleet_discount_percentage`
   - All models require `is_active`, `date_created`, `date_updated`, `id` (UUID)
...
5. **Environment Configuration**:
   - **Local Development**: PostgreSQL database `patabima_insurance` @ `127.0.0.1:5432`
   - **EC2 Production**: Uses `RDS_HOSTNAME` environment variable for AWS RDS PostgreSQL
   - **Test Environment**: Uses in-memory database (configured in settings.py)
   - All environments use PostgreSQL exclusively (no SQLite)

## Backend Environment Switching

### Quick Switch Guide

The app supports switching between local development and production backends:
...
- Use when: Testing locally, debugging backend changes
- Requires: Django server running (`python manage.py runserver 0.0.0.0:8000`)

**Switch to EC2 Production** (for testing deployed backend):

```powershell
.\deployment\backend-switching\switch-backend.ps1 -Environment ec2
```

- URL: `http://44.200.182.180` (EC2 Direct IP)
- Use when: Testing production deployment, APK builds
- Instance: `i-0d0f116005d812275`

**Switch to Production SSL** (for public release):

```powershell
.\deployment\backend-switching\switch-backend.ps1 -Environment production
```

- URL: `https://api.hugo-shopping.com` (Production SSL domain)
- Use when: Building release APKs, testing with production data
- Instance: EC2 `i-0d0f116005d812275` (us-east-1)
- Note: This is the SSL-secured domain serving the live app

### How It Works

1. **Script updates** `frontend/.env.local` with selected backend URL
2. **Environment variable**: `EXPO_PUBLIC_API_BASE_URL` is read by `DjangoAPIService.js`
3. **Service reads env**: `BASE_URL: process.env.EXPO_PUBLIC_API_BASE_URL || 'https://api.hugo-shopping.com'`
4. **Restart required**: Stop Metro (Ctrl+C), then `npx expo start --clear` to reload environment
5. **Clear app cache**: Tap version 7x on login screen to open Backend Switcher, then close modal to clear cache
6. **Backup created**: Previous config saved to `.env.local.backup`
...
# Check from emulator perspective (how app sees it)
curl http://10.0.2.2:8000/api/v1/motor2/categories/
```
...
# 4. In app: Tap version 7x on login screen → Close modal (clears cache)
# 5. Reload app: Press 'r' in Metro terminal
# 6. Verify: Backend Switcher should show http://10.0.2.2:8000
```
...
# 3. In app: Tap version 7x on login screen → Close modal (clears cache)
# 4. Reload app: Press 'r' in Metro terminal
# 5. Verify: Backend Switcher should show https://api.hugo-shopping.com
```

### Troubleshooting

**App still using old URL after switch?**

1. **Clear Metro cache**: Stop Metro (Ctrl+C), then `npx expo start --clear`
2. **Clear app cache**: Tap version 7x on login screen → Close Backend Switcher modal
3. **Force reload**: Press 'r' in Metro terminal
4. **Last resort**: Uninstall app from emulator and reinstall

**Metro not picking up environment changes?**

- Always use `npx expo start --clear` (with --clear flag) after switching
- Environment variables are read at Metro startup, not runtime
- Verify `.env.local` shows correct URL: `Get-Content frontend\.env.local`

**"Network request failed" error?**

- **Local**: Verify Django server running at `0.0.0.0:8000` (not `localhost:8000`)
- **Production**: Check internet connectivity and EC2 instance status
- **Both**: Clear app cache (tap version 7x → close modal)

**DjangoAPIService using wrong BASE_URL?**

- Check `frontend/services/DjangoAPIService.js` line 17
- Should be: `BASE_URL: process.env.EXPO_PUBLIC_API_BASE_URL || 'https://api.hugo-shopping.com'`
- Must NOT be hardcoded to specific URL
...
## Motor3 DMVIC Drawer Guide (Source of Truth)

When working on Motor3 vehicle verification (DMVIC), date overlap gating, or the “existing cover” drawer behavior, follow this guide first:

- docs/motor3/MOTOR3_DMVIC_DRAWER_FLOW_AND_TROUBLESHOOTING.md

It documents the expected UX flow, the date compliance rule (min start date = expiry + 1 day), timezone-safe parsing expectations, and a fast troubleshooting checklist to prevent regressions.

## DMVIC UAT SSL Troubleshooting

DMVIC UAT has periodically served an **expired TLS certificate**, which breaks Python `requests` with `SSLCertVerificationError`.

- Default behavior is secure: SSL verification is **ON**.
- For **local/UAT debugging only**, you can disable SSL verification for DMVIC calls:
  - Set `DMVIC_SSL_VERIFY=False`
- Safety rule:
  - Never disable SSL verification in production. Code only honors `DMVIC_SSL_VERIFY=False` when `DEBUG=True` or when `DMVIC_BASE_URL` contains `uat`.
